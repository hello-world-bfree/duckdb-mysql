...
struct ExecutionPlanCacheKey {
	...
	bool operator==(const ExecutionPlanCacheKey &other) const {
		return schema_name == other.schema_name && table_name == other.table_name &&
		       column_names == other.column_names && filter_columns == other.filter_columns &&
		       selectivities == other.selectivities;
	}
};

struct ExecutionPlanCacheKeyHash {
	size_t operator()(const ExecutionPlanCacheKey &key) const {
		size_t h = std::hash<string> {}(key.schema_name);
		h ^= std::hash<string> {}(key.table_name) << 1;
		for (const auto &col : key.column_names) {
			h ^= std::hash<string> {}(col);
		}
		for (const auto &col : key.filter_columns) {
			h ^= std::hash<string> {}(col);
		}
		for (auto sel : key.selectivities) {
			h ^= std::hash<uint64_t> {}(static_cast<uint64_t>(sel * 1000000));
		}
		return h;
	}
};
...
struct CachedExecutionPlan {
	...
	std::chrono::steady_clock::time_point last_feedback_at;
};
