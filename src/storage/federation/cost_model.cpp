...
OperationCost DefaultCostModel::TransferCost(idx_t row_count, idx_t row_width_bytes, bool innodb_compressed) const {
	...
	if (compression_aware_costs_) {
		...
		if (innodb_compressed) {
			effective_ratio = std::min(effective_ratio, compression_ratio_);
		}
		if (network_calibration_.has_network_compression) {
			effective_ratio = std::min(effective_ratio, network_calibration_.network_compression_ratio);
		}
		total_bytes *= effective_ratio;
	}

	cost.network_cost = total_bytes * params_.network_cost_per_byte;

	if (network_calibration_.is_calibrated) {
...
OperationCost DefaultCostModel::CalculateHybridCost(const MySQLTableStats &stats,
                                                    ...
                                                    vector<idx_t> &local_indices) const {
...
ExecutionPlan DefaultCostModel::ComparePlans(const MySQLTableStats &stats, const FilterAnalysisResult &filter_result,
	...
	auto hybrid_cost = CalculateHybridCost(stats, filter_result, columns, hybrid_pushed, hybrid_local);

	ExecutionStrategy best_strategy = ExecutionStrategy::PUSH_ALL_FILTERS;
	OperationCost best_cost = push_all_cost;

	// NOTE: EXECUTE_ALL_LOCALLY is currently disabled because the scanner does not
	// implement local filter execution - filters would be silently ignored, returning
	// incorrect results. Once MySQLScan applies input.filters locally, this can be
	// re-enabled by uncommenting the code below and updating the cost parameters.
	//
	...
	case ExecutionStrategy::HYBRID: {
		...
		double pushed_selectivity = 1.0;
		for (auto idx : plan.pushed_filter_indices) {
			pushed_selectivity *= filter_result.analyses[idx].estimated_selectivity;
		}
		plan.estimated_rows_from_mysql = EstimateResultRows(stats, pushed_selectivity);
		plan.estimated_final_rows = EstimateResultRows(stats, filter_result);
		break;
	}
	}
...
#ifndef NDEBUG
	plan.reasoning = "push_all=" + std::to_string(push_all_cost.Total()) + ", local_all=DISABLED" +
